...
const submit = async (event: Event) => {
    ...
    const images = [
        fichierUploader.value?.files![0] ?? projet.image,
        uploadImage1.value?.files![1] ?? projet.photo1,
        uploadImage2.value?.files![2] ?? projet.photo2,
        uploadImage3.value?.files![3] ?? projet.photo3,
        uploadLogo.value?.files![4] ?? projet.logo,
    ]

    const newProjet = projet.id
        ? await pb.collection('projets').update(projet.id, { ...projet, image: images[0], photo1: images[1], photo2: images[2], photo3: images[3], logo: images[4] })
        : await pb.collection('projets').create({ ...projet, image: images[0], photo1: images[1], photo2: images[2], photo3: images[3], logo: images[4] })
...
<h1 v-if="!projet.id">Creer le Projet</h1>

<div>

    <form method="post" @submit="submit" class="flex flex-col items-center gap-y-5" >
        <ImgPb
        v-if="projet.image"
    ...
    <div v-if="user">
        ...
        <button v-if="!projet.id" type="submit">Creer</button>
